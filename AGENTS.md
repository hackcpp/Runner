...
2. 开始前在当前版本路线图写清目标、状态和完成标准。
3. 实现后记录自动化测试、构建、人工检查结果和未执行项。
4. 只有全部完成标准满足后才标记为 `已完成`。
5. 提交后补充提交 SHA；新想法先进入 Backlog，不直接扩大当前版本。
6. 版本正式发布后，将当前版本路线图整体移入 `Roadmaps/Archive/<version>.md`，记录发布日期、发布提交或 tag 和最终验收证据，再创建下一版本路线图并更新入口指针。
