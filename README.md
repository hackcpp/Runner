...
当前版本闭环为：三关固定目标、逐关加速与复杂障碍、自动前进、三车道切换、跳跃、滑铲、动作奖励、三条命、碰撞确认后检查点恢复、关卡庆祝和最终结算。

关卡节奏：第 1 关 `ROOFTOP BASICS` 为 `360m` 蓝灰黄昏，速度 `9.4 -> 12.5`；第 2 关 `CITY RHYTHM` 为 `520m` 金色夕照，速度 `10.4 -> 14.5`；第 3 关 `SUNSET SPRINT` 为 `720m` 清晰紫蓝夜幕，速度 `11.4 -> 16.5`。每关在 `1/3` 与 `2/3` 保存检查点，目标位置有横跨三条车道的棋盘格终点线及 `3m` 无障碍净空；恢复后有 `16m` 无障碍反应区，越过终点后先减速庆祝约 `1.6s`，再进入通关结算。

版本规划入口为 [ROADMAP.md](ROADMAP.md)。日常工作只读取入口指向的当前版本路线图；已发布版本和旧验收记录按版本归档，需要追溯时再查看。
...
- `Space` / `Return`: 开始游戏
- Mac Debug 开始界面：点击 `LEVEL 1`、`LEVEL 2`、`LEVEL 3`，或按数字键 `1`、`2`、`3`，直接测试指定关卡；Release 不提供此入口
...
- 碰撞后点击 `CONTINUE`，或按 `Space` / `Return` / `R`，再从最近检查点继续
...
仅在 Player 显式传入 `-taptapCapture -captureOutput <目录>` 时启用。它使用固定种子和模拟器验证过的路径自动完成换道、跳跃与滑铲，严格遵守正常碰撞结算，以 `1920x1080`、24 fps 输出第 3 关连续帧，并额外从终点前 `0.5m` 录制到结算面板，完整保留减速庆祝过程；同时采集第 1 关生命 HUD、第 2 关检查点恢复和第 3 关通关庆祝图。普通启动及 Android 触控流程不会挂载该组件。
...
商店名称、定位文案、上传素材和发布清单统一保存在本地 `Distribution/TapTap/`。根目录 `Distribution/` 是不提交到 GitHub 的发布工作区，已由 `.gitignore` 整体排除。正式隐私政策由仓库中的 `docs/privacy/index.html` 提供，并通过 [GitHub Pages](https://hackcpp.github.io/Runner/privacy/) 公开访问；正文按 TapTap 官方模板结构结合当前 APK 实际能力编写，不适用的数据处理条款明确标注为未收集或未使用。
...
- 隐私政策已填写发布主体、联系邮箱和生效日期，按 TapTap 模板补齐 Cookie、共享转让、第三方 SDK、保存期限、用户权利、未成年人保护和法定例外，并部署为公开 HTTPS 页面。
